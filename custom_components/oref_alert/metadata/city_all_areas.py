"""Map of cities to their sub-areas."""

CITY_ALL_AREAS = {
    "אשדוד - כל האזורים": [
        "אשדוד - א,ב,ד,ה",
        "אשדוד - איזור תעשייה צפוני",
        "אשדוד - ג,ו,ז",
        "אשדוד - ח,ט,י,יג,יד,טז",
        "אשדוד - כל האזורים",
        "אשדוד -יא,יב,טו,יז,מרינה,סיט",
    ],
    "אשקלון - כל האזורים": ["אשקלון - דרום", "אשקלון - כל האזורים", "אשקלון - צפון"],
    "באר שבע - כל האזורים": [
        "באר שבע - דרום",
        "באר שבע - כל האזורים",
        "באר שבע - מזרח",
        "באר שבע - מערב",
        "באר שבע - צפון",
    ],
    "הרצליה - כל האזורים": [
        "הרצליה - גליל ים ומרכז",
        "הרצליה - כל האזורים",
        "הרצליה - מערב",
        "הרצליה - מרכז וגליל ים",
    ],
    "חדרה - כל האזורים": [
        "חדרה - כל האזורים",
        "חדרה - מזרח",
        "חדרה - מערב",
        "חדרה - מרכז",
        "חדרה - נווה חיים",
    ],
    "חיפה - כל האזורים": [
        "חיפה - כל האזורים",
        "חיפה - כרמל ועיר תחתית",
        "חיפה - מערב",
        "חיפה - נווה שאנן ורמות כרמל",
        "חיפה - קריית חיים ושמואל",
        "חיפה-מפרץ",
    ],
    "ירושלים - כל האזורים": [
        "ירושלים - אזור תעשייה עטרות",
        "ירושלים - דרום",
        "ירושלים - כל האזורים",
        "ירושלים - כפר עקב",
        "ירושלים - מזרח",
        "ירושלים - מערב",
        "ירושלים - מרכז",
        "ירושלים - צפון",
    ],
    "נתניה - כל האזורים": ["נתניה - כל האזורים", "נתניה - מזרח", "נתניה - מערב"],
    "ראשון לציון - כל האזורים": [
        "ראשון לציון - כל האזורים",
        "ראשון לציון - מזרח",
        "ראשון לציון - מערב",
    ],
    "רמת גן - כל האזורים": ["רמת גן - כל האזורים", "רמת גן - מזרח", "רמת גן - מערב"],
    "תל אביב - כל האזורים": [
        "תל אביב - דרום העיר ויפו",
        "תל אביב - כל האזורים",
        "תל אביב - מזרח",
        "תל אביב - מרכז העיר",
        "תל אביב - עבר הירקון",
    ],
}
